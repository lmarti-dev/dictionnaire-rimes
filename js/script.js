...
const CONSONNES =
  CONSONNES_ASSOC + [...CONSONNES_LETTRES].map(e => `|${e}`).join('')
const CR = `(?:${CONSONNES})`
...
function get_end_regex (pron) {
  ...
  m = pron.match(
    new RegExp(`((?:${CR}|-)+(?:${VR}|-)+$)|((?:${VR}|-)+(?:${CR}|-)+'?$)`)
  )
  if (m == null) {
    return /$/
  }
  pron_end = m[0]
  penultieme_syllabe = pron
    .slice(0, m.index)
    .match(new RegExp(`(${CR}|${VR})$`))
  pron_end_dashed = pron_end.replaceAll(
    new RegExp(`(${VR}|${CR})(?!$)`, 'g'),
    '$1-?'
  )
  ...
  let end_regex
  if (VOYELLES_DANGEREUSES.includes(pron_end_dashed[0])) {
    end_regex = `(?<!${get_forbidden_voyelles(pron_end_dashed[0])})`
  } else {
    end_regex = ''
  }
...
function heuristic_pron_find (v) {
  ...
  // s -> z
  v = v.replace(new RegExp(`(?<![scpr])s(?![shc])$`), 'z', v)
  // sc ->
  ...
  // g mou
  v = v.replaceAll(/g([íêéèyìiey])/g, 'j$1', v)
  // g dur
  v = v.replaceAll(/g(.*)/g, 'gh$1', v)
...
function append_rimes_to_content () {
  ...
  for (
    i = current_rimes;
    i < Math.min(rimes.length, current_rimes + DEFAULT_MAX_RIMES);
    i++
  ) {
  ...
  function doneTyping (force_reload = false) {
    ...
    if (search.value == '') {
      ...
      if (pron != '') {
        ...
        info_line.innerHTML = `<span class'font-italic'>${pron}</span> ― <code>${end_regex
          .toString()
          .replace(/\</, '&lt;')}</code>
        </code> ― <span>${
          rimes.length
